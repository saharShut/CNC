...
void limits_init()
{
    ...
    if (bit_istrue(settings.flags, BITFLAG_HARD_LIMIT_ENABLE))
    {
       ...
       EXTI_Init.Line = ((1 << X_LIMIT_BIT) | (1 << Y_LIMIT_BIT) | (1 << Z_LIMIT_BIT));

       EXTI_ConfigTypeDef EXTI_Conf;
       EXTI_Conf.Line = ((X_LIMIT_BIT) | (Y_LIMIT_BIT) | (Z_LIMIT_BIT));
...
uint8_t limits_get_state()
{
  ...
  uint16_t pin = HAL_GPIO_ReadPin(LIMIT_PIN, GPIO_PIN_All);
...
{
	if (EXTI->PR & (1 << X_LIMIT_BIT))
	{
          EXTI->PR &= (1 << X_LIMIT_BIT);
	}
	if (EXTI->PR & (1 << Y_LIMIT_BIT))
	{
          EXTI->PR &= (1 << Y_LIMIT_BIT);
	}
	if (EXTI->PR & (1 << Z_LIMIT_BIT))
	{
          EXTI->PR &= (1 << Z_LIMIT_BIT);
	}
