...
//#include "misc.h"
void TIM_Configuration(TIM_HandleTypeDef* TIMER, uint16_t Period, uint16_t Prescaler, uint8_t PP);
...
void st_wake_up()
{
  ...
  TIM3->EGR = (uint16_t)0x0001; //TIM_PSCReloadMode_Immediate;
...
#endif
  TIM2->EGR = (uint16_t)0x0001; //TIM_PSCReloadMode_Immediate;
...
void TIM2_IRQHandler(void)
{
  ...
  DIRECTION_PORT->ODR = ((uint16_t)DIRECTION_PORT->ODR & ~DIRECTION_MASK) | (st.dir_outbits & DIRECTION_MASK);
  __HAL_TIM_CLEAR_IT(&htim3, TIM_IT_UPDATE);
...
void TIM3_IRQHandler(void)
{
  ...
  if ((TIM3->SR & 0x0001) != 0)                  // check interrupt source
  {
    ...
    STEP_PORT->ODR = (((uint16_t)STEP_PORT->ODR) & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK);
  }
}
#ifdef STEP_PULSE_DELAY
...
void st_reset()
{
  ...
  GPIOA->ODR = (GPIOA->ODR & ~STEP_MASK & ~DIRECTION_MASK) | (step_port_invert_mask & STEP_MASK) | (dir_port_invert_mask & DIRECTION_MASK);
}
...
void stepper_init()
{
  ...
  GPIO_InitStr.Pin = STEPPERS_DISABLE_MASK;
  HAL_GPIO_Init(STEPPERS_DISABLE_PORT, &GPIO_InitStr);
  
  GPIO_InitStr.Pin = STEP_MASK;
  HAL_GPIO_Init(STEP_PORT, &GPIO_InitStr);
  
  GPIO_InitStr.Pin = DIRECTION_MASK;
  HAL_GPIO_Init(DIRECTION_PORT, &GPIO_InitStr);
  
  RCC->APB1ENR |= RCC_APB1ENR_TIM2EN;
  TIM_Configuration(&htim2, 1, 1, 1);
  RCC->APB1ENR |= RCC_APB1ENR_TIM3EN;
  TIM_Configuration(&htim3, 1, 1, 1);
